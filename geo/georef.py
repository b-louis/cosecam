...
from coscam.utils.homography import Homography
from coscam.utils.homography_new import homography
import shutil
...
def cal_lat_lon(lat1,lon1,d,tc):
    ...
    if (np.cos(lat)==0):
        ...
        lon = np.mod( lon1 - np.arcsin(np.sin(tc)*np.sin(d)/np.cos(lat))+np.pi, 2*np.pi) - np.pi
    return lat,lon
...
class StereoGCP():
    ...
    def compute_gcp(self,points,angle,coordinates):
        """
        The full function that computes GCP for one pair of points
        TODO : finish it
...
class RpcGenerator():
    """
        RPC class that encapsulates the image generation
    """
...
class Georeferencer(QObject):
    ...
    def _georef(self,data1,data2):
        ...
        print("Temps de calcul init --- %s seconds ---" % (time.time() - start_time))
        ...
        print("Temps de calcul des Gcps --- %s seconds ---" % (time.time() - start_time))
        ...
        print("Temps de calcul des Rpcs --- %s seconds ---" % (time.time() - start_time))
        ...
        print("Temps de calcul orhot --- %s seconds ---" % (time.time() - start_time))
        ...
        img1_geo, img2_geo = homography.global_homgraphy(img1_geo,img2_geo,d=0.3)
        ...
        print("Temps de calcul des homolast --- %s seconds ---" % (time.time() - start_time))
...
def convert_save_gcps(gcps,file_output):
    ...
    gcps_gdal = [gdal.GCP(new_gcps[i][0],new_gcps[i][1],0,new_gcps[i][2],new_gcps[i][3]) for i in range(nb_points) ]
    header=['pixelX','pixelY','mapX','mapY']
    header_save = ','.join(header)
    np.savetxt(file_output,new_gcps[:,[2,3,0,1]],header=header_save,fmt='%f')
